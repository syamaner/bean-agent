...
class RoastingAgent:
    ...
    def __init__(self, openai_client: OpenAI, roaster_session, detection_session, test_audio_file: Optional[str] = None):
        ...
        self.roast_log = []
        self.test_audio_file = test_audio_file  # For testing with recorded audio
    ...
    async def start_roast(self, plan: Dict[str, Any]):
        ...
        if self.test_audio_file:
            # Use recorded audio file for testing
            await self.detection.call_tool("start_first_crack_detection", {
                "audio_source_type": "audio_file",
                "audio_file_path": self.test_audio_file
            })
            print(f"   ✓ First crack detection started (using test file: {self.test_audio_file})", flush=True)
        else:
            # Use live USB microphone
            await self.detection.call_tool("start_first_crack_detection", {
                "audio_source_type": "usb_microphone"
            })
            print("   ✓ First crack detection started (using USB microphone)", flush=True)
...
async def run_autonomous_roast(roast_prompt: str, test_audio_file: Optional[str] = None):
    """Run autonomous roasting with user prompt.
    
    Args:
        roast_prompt: User prompt describing the desired roast
        test_audio_file: Optional path to audio file for testing FC detection
    """
    import logging
            ...
            async with ClientSession(roaster_read, roaster_write) as roaster_session:
                ...
                async with sse_client(DETECTION_SSE_URL, headers=headers) as (detection_read, detection_write):
                    async with ClientSession(detection_read, detection_write) as detection_session:
                        ...
                        # Create agent
                        agent = RoastingAgent(ai_client, roaster_session, detection_session, test_audio_file=test_audio_file)
...
if __name__ == "__main__":
    ...
    # Priority: CLI args > environment variable > None
    test_audio_file = os.getenv("TEST_AUDIO_FILE") or None
    if test_audio_file and not test_audio_file.strip():
        test_audio_file = None  # Treat empty string as None
    
    if len(sys.argv) > 1:
        # Check if first arg is --audio-file
        if sys.argv[1] == "--audio-file":
            if len(sys.argv) < 3:
                print("Error: --audio-file requires a file path argument", flush=True)
                sys.exit(1)
            test_audio_file = sys.argv[2]  # CLI overrides env var
            # Use remaining args as prompt (if any)
            if len(sys.argv) > 3:
    ...
    asyncio.run(run_autonomous_roast(prompt, test_audio_file=test_audio_file))
